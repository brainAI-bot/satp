...
is instruction-name aligned but not canonical for consumer construction because
the published fee-routing account schemas omit `treasury`.
...
- Canonical repository IDL: `idls/v3/escrow_v3.json`, 14 instructions,
  SHA-256 `ef9622a6d07bd818d3a74ba6c61f3b3f447f61167e82aadc65ecbce4fb307829`.
...
- Anchor 1.0 Program Metadata account
  ...
  14-instruction escrow set but omits writable `treasury` from `release` and
  `partial_release`, so consumers must fail closed for instruction construction.
...
consumers and tests compare the resolved program address, instruction names, and
fee-routing account schemas before treating any published IDL as authoritative.
Consumer escrow unpause remains false and AgentFolio money-moving routes remain
gated until a separate AgentFolio-owned unpause gate verifies product behavior.
