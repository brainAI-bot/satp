...
- Cover the minimum RC-S6 semantic uncertainty cases: positive, stale, revoked,
  ...
  and AgentFolio consumer-copy records.
...
| `score-meaning-boundary.json` | deterministic warning or fail closed | Score input that proves numeric score meaning is not release-approved for trust promotion, ranking, or eligibility. |
...
| `agentfolio-consumer-copy-boundary.json` | deterministic warning or fail closed | AgentFolio-facing copy boundary fixture that rejects mainnet, escrow, npm promotion, launch, and payment readiness claims. |
...
5. Reputation, validation, score, review, and escrow reference fixtures cannot
   be promoted into release-ready semantics unless their RC-S6 uncertainty
   status is explicit.
...
7. AgentFolio-facing copy surfaces remain consumer-only and must not imply
   mainnet readiness, escrow/value-bearing readiness, npm latest promotion,
   production dependency replacement, public launch readiness, signing, Solana
   writes, package publishing, or live payment handling.
...
| Score meaning | Score input is parseable and tied to a subject identity and formula version. | Numeric score scale, formula authority, aggregation inputs, ranking meaning, and eligibility meaning are not release-approved. | Downgrade to warning or fail closed; do not promote the number into trust score, tier, ranking, eligibility, or protected-action access. | Consumers may display it only as uncertain fixture evidence when appropriate, and must keep policy decisions unchanged unless separately approved. |
...
| AgentFolio consumer copy | Copy states offline/read-only package and consumer boundaries. | Copy implies mainnet readiness, escrow/value-bearing readiness, npm latest promotion, production dependency replacement, public launch readiness, signing, Solana writes, package publishing, or live payment handling. | Reject the copy claim or keep it as a warning-only boundary fixture; do not use SATP conformance evidence as AgentFolio product approval. | AgentFolio remains a consumer; SATP copy must not alter AgentFolio launch state, dependency policy, marketplace escrow policy, or product code. |
