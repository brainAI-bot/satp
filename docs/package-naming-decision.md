...
| Client package | `packages/satp-client/package.json` is named `@brainai/satp-client`, version `2.0.6`, `private: false`, and publishes publicly only if a future release gate authorizes another publish. | This is the SDK source package and the current stable consumer package family. |
| npm `@brainai/satp-client` | Registry readback on 2026-08-02: `latest` is `2.0.6`, `rc` is `2.0.2`, and published versions are `0.1.0-rc.0`, `2.0.0`, `2.0.1`, `2.0.2`, `2.0.3`, `2.0.4`, `2.0.5`, and `2.0.6`. Owner-approved `REQ-6b35eb58` is the 2.0.6 roadmap-gate approval authority; HQ task `SATP-NPM-PUBLISH-APPROVAL-EFFECTIVE-20260802` records the 2.0.6 publish/readback execution. Owner-approved `REQ-cc84fa3a` and PR #115 remain the historical stable-line / 2.0.2 public provenance. | Stable consumers use `@brainai/satp-client@2.0.6` or `@brainai/satp-client`; reproducible historical rc evidence uses exact `@brainai/satp-client@2.0.2`; `@rc` is only for HQ-assigned moving-tag validation. |
