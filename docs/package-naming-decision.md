...
| npm `@brainai/satp-client` | Registry readback on 2026-07-26: `latest` is `2.0.3`, `rc` is `2.0.2`, and published versions are `0.1.0-rc.0`, `2.0.0`, `2.0.1`, `2.0.2`, and `2.0.3`. HQ task `TASK-cc897dc9` records the Owner-approved 2.0.3 npm closeout, and `TASK-f53c7ecb` records source alignment via PR #107. | Stable consumers use `@brainai/satp-client@2.0.3` or `@brainai/satp-client`; reproducible historical rc evidence uses exact `@brainai/satp-client@2.0.2`; `@rc` is only for HQ-assigned moving-tag validation. |
