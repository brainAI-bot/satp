...
| `src/satp-client/sync-v3.js` | SDK/client source | `packages/satp-client/ + packages/satp-solana/ + packages/satp-core/ split` | Move protocol-owned source into `packages/satp-client/` or split pure helpers into `packages/satp-core/` and Solana helpers into `packages/satp-solana/`. |
